...
export class InputObj {
    ...
    render() {
        ...
        if ((this.avatar == "") || (this.nickName == "")) { //если выход, то блокируем область ввода и чистим
                    if (area) { area.value = ""; area.setAttribute('disabled', 'true') };
                    if (areaButton) areaButton.setAttribute("disabled", "true")
                    if (areaButton) areaButton.style.backgroundColor = "#A2A2A2";
